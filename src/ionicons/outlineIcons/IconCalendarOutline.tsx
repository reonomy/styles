import React from 'react';
import SvgIcon, { SvgIconProps } from '@material-ui/core/SvgIcon';

export default function IconCalendarSolid(props: SvgIconProps) {
  return (
    <SvgIcon viewBox="0 0 24 24" fill="none" {...props}>
      <path
        d="M15 10.875C15 11.4963 14.4963 12 13.875 12C13.2537 12 12.75 11.4963 12.75 10.875C12.75 10.2537 13.2537 9.75 13.875 9.75C14.4963 9.75 15 10.2537 15 10.875Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M17.625 12C18.2463 12 18.75 11.4963 18.75 10.875C18.75 10.2537 18.2463 9.75 17.625 9.75C17.0037 9.75 16.5 10.2537 16.5 10.875C16.5 11.4963 17.0037 12 17.625 12Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M15 14.625C15 15.2463 14.4963 15.75 13.875 15.75C13.2537 15.75 12.75 15.2463 12.75 14.625C12.75 14.0037 13.2537 13.5 13.875 13.5C14.4963 13.5 15 14.0037 15 14.625Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M17.625 15.75C18.2463 15.75 18.75 15.2463 18.75 14.625C18.75 14.0037 18.2463 13.5 17.625 13.5C17.0037 13.5 16.5 14.0037 16.5 14.625C16.5 15.2463 17.0037 15.75 17.625 15.75Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M7.5 14.625C7.5 15.2463 6.99632 15.75 6.375 15.75C5.75368 15.75 5.25 15.2463 5.25 14.625C5.25 14.0037 5.75368 13.5 6.375 13.5C6.99632 13.5 7.5 14.0037 7.5 14.625Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M10.125 15.75C10.7463 15.75 11.25 15.2463 11.25 14.625C11.25 14.0037 10.7463 13.5 10.125 13.5C9.50368 13.5 9 14.0037 9 14.625C9 15.2463 9.50368 15.75 10.125 15.75Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M7.5 18.375C7.5 18.9963 6.99632 19.5 6.375 19.5C5.75368 19.5 5.25 18.9963 5.25 18.375C5.25 17.7537 5.75368 17.25 6.375 17.25C6.99632 17.25 7.5 17.7537 7.5 18.375Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M10.125 19.5C10.7463 19.5 11.25 18.9963 11.25 18.375C11.25 17.7537 10.7463 17.25 10.125 17.25C9.50368 17.25 9 17.7537 9 18.375C9 18.9963 9.50368 19.5 10.125 19.5Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        d="M15 18.375C15 18.9963 14.4963 19.5 13.875 19.5C13.2537 19.5 12.75 18.9963 12.75 18.375C12.75 17.7537 13.2537 17.25 13.875 17.25C14.4963 17.25 15 17.7537 15 18.375Z"
        fill="black"
        fillOpacity="0.54"
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M6 1.5C6.41421 1.5 6.75 1.83579 6.75 2.25V3H17.25V2.25C17.25 1.83579 17.5858 1.5 18 1.5C18.4142 1.5 18.75 1.83579 18.75 2.25V3H19.5C21.1569 3 22.5 4.34315 22.5 6V19.5C22.5 21.1569 21.1569 22.5 19.5 22.5H4.5C2.84315 22.5 1.5 21.1569 1.5 19.5V6C1.5 4.34315 2.84315 3 4.5 3H5.25V2.25C5.25 1.83579 5.58579 1.5 6 1.5ZM19.5 8.25C20.25 8.25 21 9 21 9.75V19.5C21 20.3284 20.3284 21 19.5 21H4.5C3.67157 21 3 20.3284 3 19.5V9.75C3 9 3.75 8.25 4.5 8.25H19.5Z"
        fill="black"
        fillOpacity="0.54"
      />
    </SvgIcon>
  );
}
